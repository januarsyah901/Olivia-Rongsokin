...
- Layar onboarding menampilkan dua pilihan role: **Customer** atau **Pengepul**. Pilihan ini disimpan pada field `role` di tabel `users` dan bersifat permanen (tidak bisa diubah sendiri tanpa intervensi Admin, untuk menjaga integritas data).
...
|---|---|---|
| `PENDING` | Pesanan dibuat Customer, menunggu respons Pengepul. | Customer submit form pesanan |
| `CONFIRMED` | Pengepul menerima pesanan. | Pengepul klik 'Terima' |
...
| `CANCELLED` | Pesanan dibatalkan oleh salah satu pihak. | Pengepul klik 'Tolak' / Customer batalkan |
...
| `orders` | `id, customer_id, collector_id, category_id, method (ENUM: pickup/dropoff), photo_url, estimated_weight, status (ENUM), actual_weight, agreed_price, total_price, notes, created_at` | Tabel inti transaksi. Status mengikuti state machine OMS |
